...
def FirstFigure(Game):
	...
	templategresit=cv2.cvtColor(img_rgb[ScaleY(circlecentery-circleradius/2):ScaleY(circlecentery+circleradius/2),ScaleX(circlecenterx-circleradius/2):ScaleX(circlecenterx+circleradius/2)], cv2.COLOR_BGR2GRAY)
	templatecorrect=cv2.cvtColor(img_rgb[ScaleY(SquarePoint1[1]):ScaleY(SquarePoint2[1]),ScaleX(SquarePoint1[0]):ScaleX(SquarePoint2[0])], cv2.COLOR_BGR2GRAY)
	...
	while(imageFindonCamera(Game,templatecorrect,ScaleX(SquarePoint1[0]),ScaleY(SquarePoint1[1]),60,0.8)==1):
		Game.timer-=1
		if(imageFindonCamera(Game,templategresit,ScaleX(circlecenterx-circleradius/2),ScaleY(circlecentery-circleradius/2),60,0.6)==0):
			Game.state='CLEAN'
			Game.timer=0
			return Game.state
